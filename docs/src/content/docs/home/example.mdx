...
client.on("raw", (d) => client.lavalink.sendRawData(d)); // send raw data to the client to handle stuff
...
```ts
import { QueueChangesWatcher, LavalinkManager } from "nerox-llclient";
...
Full code sample: can be found on the [Testbot in here](https://github.com/tanmayy91/lava-client/blob/main/testBot/Utils/handleResuming.ts)
